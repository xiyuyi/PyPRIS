...
ticket.distortion_1_amp = 0.813 # this is the y dimension (vertical). CL causes field compression
ticket.distortion_1_shift = 9.459
ticket.shift_channel_shift1 = -2.81
ticket.shift_channel_shift2 = -2
ticket.psfz0_CL = 40  # the count of the center plane, define it as z=0 in the psf coordinates system.
ticket.psfz0_shift = 40  # the count of the center plane, define it as z=0 in the psf coordinates system.

"where to find the data files, for both blur and observation"
paths = []
paths.append('G:\\DH_localization\\EPFL_datasets\\N2')
ax0_ranges = []
ax0_ranges.append(list([-10, 10]))
ticket_folders = []
ticket_folders.append('PyPRIS_CLnShift_set1')
...
for datapath, ax0_range, ticket_folder in zip(paths, ax0_ranges, ticket_folders):
    ...
    ticket.init_ax0_range = list([-10, 10])
    ticket.init_ax1_range = list([1, 160])
