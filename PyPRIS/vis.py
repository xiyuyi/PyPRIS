...
def pris_show(locs, candidate_locs, lb, outputhtml, saveoption):
    ...
    p5 = figure(x_range=p1.x_range, y_range=p1.y_range,
                tooltips=[("x", "$x"), ("y", "$y"), ("value", "@image")],
                title='blur + last pris support')
    p5.x_range.range_padding = p3.y_range.range_padding = 0
    p5.image(image=[blur], x=0, y=0, dw=w, dh=h)
    try:
        x = candidate_locs
        p5.scatter(x[:, 2], x[:, 1], marker='dot', size=25, color='red')
        p5.scatter(locs[:, 1], locs[:, 0], marker='dot', size=25, color='blue')
    ...
    p6 = figure(x_range=p1.x_range, y_range=p1.y_range,
                tooltips=[("x", "$x"), ("y", "$y"), ("value", "@image")],
                title='final pris reconstruction + final fits')
...
def pris_show_pris0to4(locs, cands0, cands1, cands2, cands3, cands4, lb, outputhtml, saveoption):
    ...
    p2.image(image=[blur], x=0, y=0, dw=w, dh=h)
    try:
    ...
    p4.image(image=[blur], x=0, y=0, dw=w, dh=h)
    try:
    ...
    p6.image(image=[blur], x=0, y=0, dw=w, dh=h)
    try:
